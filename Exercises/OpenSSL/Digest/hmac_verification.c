...
int main(int argc, char **argv){
    ...
    // VERIFICATION
    unsigned char hmac[] = "cfd2d965486f7067e082b41ca4e2ed810cfb249a";    // hex
    unsigned char hmac_binary[strlen(hmac)/2];

    for (int i = 0; i < strlen(hmac)/2; i++)
        sscanf(&hmac[2*i], "%2hhx", &hmac_binary[i]);

    // lenght actual comparison of the buffers
...
// For verification successfull, run the following commands:
// ./hmac_verification.exe hmac2.c
// otherwise, use any other file